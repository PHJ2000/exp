...
4. 전송하고 싶으면 `보내`나 `완료`라고 말하면 Enter가 눌립니다.

추천 흐름:

1. 문장을 말해서 먼저 입력합니다.
2. 틀렸으면 `지워`, `방금 지워`, `전부 지워`, `다시 OO`처럼 말합니다.
3. 맞으면 `보내`라고 말해서 제출합니다.
...
- 음성 명령
  - `보내`, `보내줘`, `완료`, `완료해줘`
  - `취소`, `지워`, `지어`, `치워`, `방금 지워`, `마지막 지워`
  - `전부 지워`, `다 지워`, `전체 지워`, `모두 지워`, `싹 지워`
  - `다시 OO`
...
- `전부 지워` 계열은 아직 엔터 치지 않은 현재 입력 줄 전체를 지웁니다.
- 이미 `보내`나 `완료`로 제출된 문장은 안전하게 되돌리기 어렵기 때문에, 보통은 `문장 -> 다시 말하기 -> 보내` 흐름이 가장 안정적입니다.
